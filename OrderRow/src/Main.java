	...
	public static void main(String[] args) {
		...
		while(go&&HP>0) 
		{
			...
			while(tempDeck.size()>0) 
			{
				...
				while (choice2 < 1 || choice2 > tempDeck.size()) {
			...
			for(int m=0; m<map.size()&HP>0; m++)
			{
				...
				if(part.equals("shop")) 
				{
					...
					Card sale1 = new Card(r.nextInt(1,13)); //TODO new Card(r.nextInt(1,8));
					Card sale2 = new Card(r.nextInt(1,13));
					Card sale3 = new Card(r.nextInt(1,13));
				...
				if(part.equals("random")) 
				{
					...
					if(random==3)
					{
						Card free1 = new Card(r.nextInt(1,13)); //TODO new Card(r.nextInt(1,8));
						Card free2 = new Card(r.nextInt(1,13));
						Card free3 = new Card(r.nextInt(1,13));

						System.out.println("Oh, goodie! Somebody left these unattended!");
						System.out.println("1): Steal "+free1.toString());
						System.out.println("2): Steal "+free1.toString());
						System.out.println("3): Steal "+free1.toString());
						System.out.println("4): Leave.");
						System.out.println();
						System.out.println("Type # of your choice:");
						int choice = in.nextInt();
						...
						if (choice==4)
						{
							System.out.println("You're a good samaritan!");
						}
				...
				if(part.equals("fight")) 
				{
					...
					while(!allDead(opp)&&HP>0) 
					{
						...
						if(pick.getDMG()>0&&!pick.isHitAll()) 
						{
							...
							if(pick.getBurn()>0)
							{
								System.out.println(opp.get(target).getName()+ " is burnt for "+ RED +pick.getBurn()+ " turns"+WHITE);
								opp.get(target).addTime((pick.getBurn()));
							}
